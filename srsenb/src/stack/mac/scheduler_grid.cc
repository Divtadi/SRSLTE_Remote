...
alloc_outcome_t sf_grid_t::alloc_ul_data(sched_ue* user, ul_harq_proc::ul_alloc_t alloc, bool needs_pdcch)
{
  ...
  std::cout<<"This is the newmask fill value: "<< newmask.to_string()<<std::endl<<"/n";
 ...
 alloc_outcome_t sf_grid_t::alloc_ul_data(sched_ue* user, ul_harq_proc::ul_alloc_t alloc, bool needs_pdcch) {
    std::cout << "Divya Allocation outcome @alloc value in grid: " << alloc.L << std::endl << "/n";
        if (user != nullptr) {
            ...
            std::cout << "This is the QCI value in the grid file inside UL code: " << user->get_qci() << std::endl
                      << "/n";
            if (user->get_qci() == 7) {
                newmask.fill(alloc.RB_start, alloc.RB_start + alloc.L / 2);
                std::cout << "This is the new mask QCI 7 after alloc: " << newmask.to_string() << std::endl << "/n";
...
void sf_sched::generate_sched_results(sf_sched_result* sf_result)
{
  ...
  sf_result->ul_mask    = tti_alloc.get_ul_mask(); //this is printing 110000.......0011
